...
If an older deployment shows `Cannot access ... before initialization`, replace `index.html` and `app.js` with the current versions in this package and commit the changes. The current version uses the browser UMD build of pdf-lib and the unminified PDF.js browser build to avoid the CDN module/minification issue. After GitHub Pages redeploys, use Ctrl+F5 to force-refresh the site.


### Fix 2026-08-24 v2
PDF.js cleanup now uses the loading task when available rather than assuming the document proxy exposes `destroy()`. This fixes the `doc.destroy is not a function` error. `index.html` also cache-busts `app.js` so GitHub Pages browsers receive the updated script.
