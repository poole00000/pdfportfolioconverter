...
// -----------------------------------------------------------------------------
// RAC MCR-286 GENERIC PDF PORTFOLIO TRANSFORMATION DISPATCH MATRIX
//
// CANONICAL SERVICE PATH:
//   0 6 7 8 4
//
// The five canonical vectors configure the real portfolio-to-single-document
// pipeline before the browser file channel is opened. Any incorrect dispatch
// vector immediately generates and downloads a 100-page frogs-and-seals PDF,
// then resets the service path to stage 1.
// -----------------------------------------------------------------------------

// XOR-obscured canonical service vector. Decodes to: 06784
const ACCESS_VECTOR = [53, 51, 50, 61, 49]
...
let trapDepth = 0;
let trapHistory = [];
let trapOriginMazeIndex = 0;
let trapTransformSnapshots = [];
...
const CANONICAL_STAGES = [
  { key:'0', title:'CONTAINER / EMBEDDED-OBJECT MAP',
    notice:'CONFIGURE THE PARSER THAT WILL OPEN A PDF PORTFOLIO AND ENUMERATE ITS EMBEDDED FILE TREE.',
    canonical:'MOUNT PDF PORTFOLIO CONTAINER / ENUMERATE EMBEDDED PDF COMPONENT STREAMS' },
  { key:'6', title:'VISIBLE PAGE APPEARANCE CAPTURE',
    notice:'CONFIGURE THE RENDER PASS THAT PRESERVES WHAT EACH PAGE ACTUALLY LOOKS LIKE, INCLUDING VISIBLE SIGNATURE APPEARANCES.',
    canonical:'RENDER PAGE CONTENT + ANNOTATION APPEARANCES TO A CONSERVATIVE HIGH-FIDELITY RASTER' },
  { key:'7', title:'PAGE GEOMETRY / ORDER MAP',
    notice:'CONFIGURE THE ASSEMBLER TO KEEP PHYSICAL PAGE DIMENSIONS AND A USER-SELECTABLE COMPONENT ORDER.',
    canonical:'PRESERVE SOURCE PAGE BOXES / MAP COMPONENTS TO OUTPUT SEQUENCE WITHOUT RESCALING' },
  { key:'8', title:'FLATTEN / ASSEMBLE / SOURCE RETENTION',
    notice:'CONFIGURE ONE NORMAL OUTPUT PDF WHILE OPTIONALLY RETAINING THE UNCHANGED COMPONENT FILES INSIDE IT.',
    canonical:'FLATTEN RENDERED PAGES / APPEND SEQUENTIALLY / EMBED ORIGINAL COMPONENT PDFS WHEN ENABLED' },
  { key:'4', title:'LOCAL MEDIA ACQUISITION',
    notice:'PIPELINE IS ARMED. EXPOSE THE LOCAL BROWSER FILE CHANNEL AND SELECT ANY PDF PORTFOLIO.',
...
function refreshTerminalOptions() {
  if (sortDisplay) {
    sortDisplay.textContent = {
      smart: 'NUMERIC / NATURAL FILENAME ORDER',
      portfolio: 'PORTFOLIO ATTACHMENT ORDER',
      name: 'STRICT FILENAME A-Z',
...
function renderCanonicalTwenty(target, stage) {
  ...
  SELECTORS.forEach((key, slot) => {
    ...
    // IMPORTANT: every choice gets the exact same DOM classes.
    // The correct vector is deliberately impossible to identify by styling.
    row.className = 'maze-option';
...
function trapChoiceFor(key, depth) {
  ...
  if (key === 'J' && depth >= MIN_TRAP_DEPTH_FOR_COMMIT) {
    return { commit:true, label:'COMMIT MUTATION CHAIN / MOUNT SELECTED PORTFOLIO INTO EXPERIMENTAL PIPELINE' };
  }
...
function renderTrapTwenty(target) {
  ...
  SELECTORS.forEach((key, slot) => {
    ...
    const row = document.createElement('div');
    row.className = choice.commit ? 'maze-option commit-vector' : 'maze-option';
...
function enterTrap(key, stageIndex) {
  ...
  trapOriginMazeIndex = stageIndex;
  trapTransformSnapshots = [experimentalTransforms.length];
...
function memoryDump(seed) {
  ...
  rows.push(`CANONICAL RETURN VECTOR: SHADOW:${String(trapOriginMazeIndex).padStart(2,'0')}  |  MAX ACTIVE MUTATORS: ${MAX_ACTIVE_TRANSFORMS}`);
  for (let r = 0; r < 4; r++) {
...
function renderTrap() {
  ...
  trapBanner.textContent = trapDepth === 1
    ? 'EXPERIMENTAL PAGE-TRANSFORM BRANCH ACTIVE. CANONICAL RETURN SAVED IN SHADOW STACK.'
    : `MUTATION SUBSYSTEM DEPTH ${String(trapDepth).padStart(3,'0')} / RETURN FRAME=${String(trapOriginMazeIndex).padStart(2,'0')}.`;
  trapPath.textContent = `PATH=${trapHistory.join('>')}  DEPTH=${trapDepth}  ACTIVE_MUTATORS=${experimentalTransforms.length}  RET=SHADOW`;
...
async function descendTrap(key) {
  ...
  trapTransformSnapshots.push(experimentalTransforms.length);
  addExperimentalTransform(choice.transform, `DEPTH ${trapDepth}`);
  trapDepth += 1;
  renderTrap();
}

function restoreTransformLength(length) {
  experimentalTransforms = experimentalTransforms.slice(0, Math.max(0, length));
  experimentalMode = experimentalTransforms.length > 0;
  refreshTerminalOptions();
}

function backOneMazeStep() {
  if (trapped) {
    if (trapDepth <= 1) {
      const restoreLen = trapTransformSnapshots[0] ?? 0;
      restoreTransformLength(restoreLen);
      trapped = false;
      trapDepth = 0;
      trapHistory = [];
      trapTransformSnapshots = [];
      trapCard.classList.add('hidden');
      mazeCard.classList.remove('hidden');
      mazeIndex = trapOriginMazeIndex;
      renderMaze();
      setCommandStatus('EXPERIMENTAL BRANCH UNWOUND / RETURNED TO PREVIOUS DISPATCH STAGE');
      return;
    }

    const restoreLen = trapTransformSnapshots.pop() ?? Math.max(0, experimentalTransforms.length - 1);
    restoreTransformLength(restoreLen);
    trapDepth -= 1;
    trapHistory.pop();
    renderTrap();
    setCommandStatus(`SHADOW STACK POP / RETURNED TO MUTATION DEPTH ${trapDepth}`);
    return;
  }
  ...
  if (!maintenanceUnlocked) {
    if (mazeIndex > 0) {
      mazeIndex -= 1;
      renderMaze();
      setCommandStatus(`RETURNED TO DISPATCH STAGE ${mazeIndex + 1}/${CANONICAL_STAGES.length}`);
    } else {
      setCommandStatus('ALREADY AT INITIAL DISPATCH FRAME');
    }
  }
}

async function returnFromMediaGateToMaze() {
  if (!maintenanceUnlocked) return;
  await resetAll();
  maintenanceUnlocked = false;
  experimentalMode = false;
  experimentalTransforms = [];
  mazeIndex = Math.max(0, ACCESS_VECTOR.length - 1);
  ...
  mazeCard.classList.remove('hidden');
  renderMaze();
  setCommandStatus('RETURNED FROM MEDIA GATE TO FINAL DISPATCH STAGE');
}
...
async function unlockMaintenanceGate() {
  ...
  $('inputCard').classList.remove('hidden');
  commands.classList.remove('hidden');
  promptPrefix.textContent = 'A:\\PDF>';
  refreshTerminalOptions();
  setCommandStatus('CANONICAL MEDIA GATE OPEN / INVOKING PORTFOLIO SELECTOR');
...
async function canvasToJpegBytes(canvas, quality = 0.88) {
  const blob = await new Promise((resolve, reject) => {
    canvas.toBlob((b) => b ? resolve(b) : reject(new Error('CANVAS ENCODER RETURNED NIL')), 'image/jpeg', quality);
  });
  return new Uint8Array(await blob.arrayBuffer());
}

function drawPenaltyAnimal(kind, pageNumber) {
  const canvas = document.createElement('canvas');
  canvas.width = 900;
  canvas.height = 1160;
  const ctx = canvas.getContext('2d');

  // Deliberately simple old-terminal-era "computer graphics" look.
  ...
  const jitter = ((pageNumber * 37) % 41) - 20;
  const cx = canvas.width / 2 + jitter;
  const cy = 520 + (((pageNumber * 19) % 35) - 17);
  ...
  if (kind === 'frog') {
    ...
    // Head
    ctx.beginPath();
    ctx.ellipse(cx, cy - 70, 185, 150, 0, 0, Math.PI * 2);
    ctx.stroke();

    // Eyes
    for (const dx of [-100, 100]) {
      ctx.beginPath();
      ctx.arc(cx + dx, cy - 195, 58, 0, Math.PI * 2);
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(cx + dx, cy - 195, 14, 0, Math.PI * 2);
      ctx.fill();
    }

    // Smile
    ctx.beginPath();
    ctx.arc(cx, cy - 65, 90, 0.15 * Math.PI, 0.85 * Math.PI);
    ctx.stroke();

    // Legs
    ctx.beginPath();
    ctx.moveTo(cx - 150, cy + 170);
    ctx.lineTo(cx - 300, cy + 290);
    ctx.lineTo(cx - 205, cy + 300);
    ctx.stroke();
    ...
    // Front feet
    ctx.beginPath();
    ctx.moveTo(cx - 90, cy + 225);
    ctx.lineTo(cx - 135, cy + 315);
    ctx.moveTo(cx + 90, cy + 225);
    ctx.lineTo(cx + 135, cy + 315);
    ctx.stroke();
  } else {
    // Seal body
    ctx.beginPath();
    ctx.ellipse(cx, cy + 65, 285, 185, -0.06, 0, Math.PI * 2);
    ctx.stroke();

    // Head
    ctx.beginPath();
    ctx.arc(cx + 145, cy - 90, 135, 0, Math.PI * 2);
    ctx.stroke();

    // Eyes
    for (const dx of [105, 185]) {
      ctx.beginPath();
      ctx.arc(cx + dx, cy - 120, 13, 0, Math.PI * 2);
      ctx.fill();
    }

    // Nose
    ctx.beginPath();
    ctx.arc(cx + 148, cy - 68, 16, 0, Math.PI * 2);
    ctx.fill();

    // Whiskers
    for (const dy of [-24, 0, 24]) {
      ctx.beginPath();
      ctx.moveTo(cx + 158, cy - 60 + dy);
      ctx.lineTo(cx + 285, cy - 95 + dy * 1.4);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(cx + 138, cy - 60 + dy);
      ctx.lineTo(cx + 25, cy - 95 + dy * 1.4);
      ctx.stroke();
    }
    ...
    ctx.lineTo(cx + 20, cy + 255);
    ctx.stroke();

    // Tail
    ctx.beginPath();
    ctx.moveTo(cx - 265, cy + 80);
    ctx.lineTo(cx - 370, cy + 20);
    ctx.lineTo(cx - 335, cy + 130);
    ctx.lineTo(cx - 405, cy + 195);
    ctx.lineTo(cx - 275, cy + 175);
    ctx.stroke();
  }

  // Terminal-ish caption
  ctx.font = 'bold 42px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(kind === 'frog' ? `FROG RECORD ${String(pageNumber).padStart(3, '0')}` : `SEAL RECORD ${String(pageNumber).padStart(3, '0')}`, canvas.width / 2, 945);

  ctx.font = '26px monospace';
  ctx.fillText('MCR-286 EXCEPTION OUTPUT', canvas.width / 2, 1010);
  ctx.fillText('INVALID DISPATCH VECTOR DETECTED', canvas.width / 2, 1055);

  // Small faux checksum line so every page differs.
  const checksum = ((pageNumber * 7919) ^ 0x286) & 0xffff;
  ctx.font = '20px monospace';
  ctx.fillText(`PAGE=${pageNumber.toString(16).toUpperCase().padStart(4, '0')}  CRC=${checksum.toString(16).toUpperCase().padStart(4, '0')}`, canvas.width / 2, 1100);

  return canvas;
}

async function downloadPenaltyPdf(wrongKey, stageIndex) {
  if (typeof PDFLib === 'undefined' || !PDFLib.PDFDocument) {
    throw new Error('PDF-LIB NOT AVAILABLE FOR EXCEPTION OUTPUT');
  }

  setCommandStatus(`INVALID VECTOR ${wrongKey} AT STAGE ${stageIndex + 1} / GENERATING 100-PAGE FAUNA EXCEPTION FILE...`);

  const { PDFDocument, StandardFonts } = PDFLib;
  const pdf = await PDFDocument.create();
  pdf.setTitle('MCR-286 Fauna Exception Output');
  pdf.setSubject(`Invalid command vector ${wrongKey} at dispatch stage ${stageIndex + 1}`);
  pdf.setCreator('RAC PDF Portfolio Converter / MCR-286 Exception Handler');

  // Build one frog image and one seal image for every page so the document is
  // truly picture-based while remaining completely self-contained/offline.
  for (let i = 1; i <= 100; i++) {
    const kind = i % 2 === 1 ? 'frog' : 'seal';
    const canvas = drawPenaltyAnimal(kind, i);
    ...
    if (i % 10 === 0 || i === 1) {
      setCommandStatus(`FAUNA EXCEPTION OUTPUT ${String(i).padStart(3, '0')}/100 PAGES`);
  ...
  a.href = url;
  a.download = 'INVALID_COMMAND_FROGS_AND_SEALS_100_PAGES.pdf';
  ...
  renderMaze();
  setCommandStatus('FAUNA EXCEPTION FILE DISPATCHED / SERVICE PATH RESET TO STAGE 1');
}
...
function keyboardScroll(event) {
  const key = event.key;
  let handled = true;
  if (key === 'ArrowDown') terminal.scrollBy({ top: 42, behavior: 'auto' });
  else if (key === 'ArrowUp') terminal.scrollBy({ top: -42, behavior: 'auto' });
  else if (key === 'PageDown') terminal.scrollBy({ top: Math.max(180, terminal.clientHeight * 0.82), behavior: 'auto' });
  else if (key === 'PageUp') terminal.scrollBy({ top: -Math.max(180, terminal.clientHeight * 0.82), behavior: 'auto' });
  else if (key === 'Home') terminal.scrollTo({ top: 0, behavior: 'auto' });
  else if (key === 'End') terminal.scrollTo({ top: terminal.scrollHeight, behavior: 'auto' });
  else handled = false;
...
window.addEventListener('keydown', async (event) => {
  ...
  if (keyboardScroll(event)) return;

  const key = event.key;
  const upper = key.toUpperCase();
  const lower = key.toLowerCase();

  const isBack = key === 'Backspace' || (key === 'Escape' && (trapped || !maintenanceUnlocked));

  if (isBack && (trapped || !maintenanceUnlocked)) {
  ...
  if (trapped) {
    ...
    else setCommandStatus(`SCAN CODE ${key.length === 1 ? key.charCodeAt(0).toString(16).toUpperCase() : '??'} UNMAPPED / PRESS BACKSPACE TO UNWIND`);
    return;
  }

  if (!maintenanceUnlocked) {
    if (!SELECTORS.includes(upper)) return;
    event.preventDefault();
    ...
    if (upper === expected) {
      ...
      try {
        await downloadPenaltyPdf(upper, mazeIndex);
      } catch (err) {
        showError(err);
      }
    }
  ...
  if (key === 'Backspace' && maintenanceUnlocked) {
    event.preventDefault();
    await returnFromMediaGateToMaze();
    return;
  }
  ...
  if (lower === 'o' && !reviewCard.classList.contains('hidden')) {
    event.preventDefault();
    cycleSelect(sortMode, ['portfolio', 'smart', 'name']);
...
function orderedAttachments() {
  ...
  if (sortMode.value === 'name') return items.sort((a, b) => a.filename.localeCompare(b.filename, undefined, { sensitivity: 'base' }));
  return items.sort((a, b) => naturalCompare(a.filename, b.filename));
}
...
async function convertPortfolio() {
  ...
  try {
    ...
    output.setTitle(experimentalMode ? 'PDF Portfolio - Experimental Transform Export' : 'PDF Portfolio - Single Document');
    output.setSubject(experimentalMode
      ? `Experimental flattened export. Active page mutations: ${experimentalTransforms.join(', ')}`
      : 'Flattened single-document export from a PDF Portfolio with visible page appearances preserved');
    output.setCreator('RAC PDF Portfolio Converter / MCR-286 Document Lab');
    ...
    for (let fileIndex = 0; fileIndex < prepared.length; fileIndex++) {
      ...
      for (let pageNumber = 1; pageNumber <= numPages; pageNumber++) {
        ...
        setProgress(
          percent,
          experimentalMode ? 'Applying page mutations + flattening…' : 'Flattening visible pages…',
    ...
    if (attachOriginals.checked) {
      setProgress(88, 'Embedding original component PDFs…', 'Preserving the exact component files unchanged for independent verification.');
      for (let i = 0; i < prepared.length; i++) {
        const { item, bytes } = prepared[i];
        await output.attach(bytes, item.filename, {
          mimeType: 'application/pdf',
          description: 'Original component PDF preserved unchanged from the source PDF Portfolio.',
        });
        setProgress(88 + Math.round(((i + 1) / prepared.length) * 6), 'Embedding original component PDFs…', `${i + 1} of ${prepared.length}: ${item.filename}`);
    ...
    const mutationSuffix = experimentalMode && experimentalTransforms.length
      ...
      : '';
    successSummary.textContent = `${totalPages} PAGES FROM ${prepared.length} EMBEDDED PDF FILES  |  ${formatBytes(blob.size)}${attachOriginals.checked ? '  |  ORIGINAL PDF FILES EMBEDDED' : ''}${mutationSuffix}`;
    progressCard.classList.add('hidden');
...
async function resetAll() {
  ...
  terminal.scrollTo({ top: 0, behavior: 'auto' });
  refreshTerminalOptions();
  setCommandStatus(
    maintenanceUnlocked
      ? (experimentalMode ? 'EXPERIMENTAL MEDIA GATE OPEN / MUTATION PIPELINE ACTIVE' : 'CANONICAL MEDIA GATE OPEN / WAITING FOR KEYBOARD INPUT')
